...
class AtlasDataset:
    ...
    def initialize(self, batch_correction=False, tree=None):
        ...
        self.readData()
        self.dataset_names = [
            "{}".format(self.ref_filename.split(".")[0]),
            "{}".format(self.target_filename.split(".")[0]),
        ]
        all_data = ad.concat(
            {self.dataset_names[0]: self.ref_data, self.dataset_names[1]: self.target_data}, axis=0, label="dataset"
        )
        ...
        branch_dic = {node.tag: [node.tag for node in self.tree.leaves(node.identifier)] for node in branch_ct}
        ...
        all_data.obs["cell_label"] = pd.Categorical(
            all_data.obs["cell_type"].to_numpy(), categories=self.all_cell_dic
        ).codes
        ...
        target_partial_Y = self.generateCandidateLabels(
            all_data.obs["cell_label"][target_index], self.ref_cell_dic, self.all_cell_dic, branch_dic
        )
    ...
    def scHPLTreeBuild(self, data):
        ...
        schpltree, _ = learn_tree(
            data=data,
            cell_type_key="cell_type",
            batch_key="dataset",
            batch_order=dataset_order,
    ...
    while len(schpltree) > 0:
        ...
        if len(node.descendants) != 0:
            for child in node.descendants:
                try:
                    tree.create_node(
                        identifier=child.name[0], parent=node.name[0], data=CellTypeNode(is_ct=True, type=child.name[0])
                    )
