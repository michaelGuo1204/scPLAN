...
def hierarchical_F1(true_labels, pred_labels, tree: TreeNode):
...
def heatmap(
    ...
    **kwargs,
    ...
    if annot:
        sns.heatmap(
            conf2.loc[order_rows, order_cols],
            vmin=0,
            vmax=1,
            cbar_kws={"label": "Fraction"},
            cmap=cmap,
            annot=conf.iloc[order_rows, order_cols],
            **kwargs,
        )
    else:
        sns.heatmap(
            conf2.loc[order_rows, order_cols], vmin=0, vmax=1, cbar_kws={"label": "Fraction"}, cmap=cmap, **kwargs
        )
