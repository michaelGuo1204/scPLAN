...
def rename_node(old_name, new_name: str, tree: TreeNode):
...
def remove_node(name, tree: TreeNode, children: bool = True):
...
def add_node(name: str, tree: TreeNode, parent: str, children: list = None):
...
def print_tree(tree: TreeNode, new_nodes: list = []):
    """Print the tree
    ...
    _print_node(tree[0], hor=0.05, ver_steps=ver_steps, fig=fig, new_nodes=new_nodes)
...
def _print_node(node, hor, ver_steps, fig, new_nodes):
    ...
    fig.text(hor, ver, txt, size=10, ha="left", va="center", bbox=dict(boxstyle="round", fc="w", ec="k"))
    ...
    # Add vertical line
    x, y = ([np.max([0.05, hor - 0.045]), np.max([0.05, hor - 0.045])], [ver_line_start, ver_line_end])
    line = mlines.Line2D(x, y, lw=1)
    fig.add_artist(line)
