...
def sankey(
    ...
    df = pd.DataFrame(
        {"left": left, "right": right, "leftWeight": leftWeight, "rightWeight": rightWeight}, index=range(len(left))
    )
    ...
    if colorDict is None:
        ...
        if missing:
            raise RuntimeError("colorDict specified but missing values: " "{}".format(",".join(missing)))
