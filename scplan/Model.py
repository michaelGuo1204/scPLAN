...
class Model(LightningModule):
    ...
    def encoderKUpdate(self):
        ...
        """
        for param_q, param_k in zip(self.encoder_q.parameters(), self.encoder_k.parameters()):
            param_k.data = param_k.data * self.param.moco_m + param_q.data * (1.0 - self.param.moco_m)
    ...
    def forward(self, in_q, in_k=None, partial_Y=None, cluster_Y=None, dataset=None):
        ...
        _, pseudo_labels_q = torch.max(predicted_scores, dim=1)  # Get pseudo labels
        ...
        for latent, label in zip(q_latent, cluster_cls_assign):
            ...
            prototype[target_label] = (
                prototype[target_label] * self.param.proto_m + (1 - self.param.proto_m) * latent  # Update prototypes
            )
        self.prototypes = torch.nn.functional.normalize(prototype, p=2, dim=1)  # Prototypes shall be normalized
        ...
        features = torch.cat((q_latent, k_latent, self.queue.clone().detach()), dim=0)  # Combine all latents/labels
