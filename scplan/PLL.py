...
from scplan.Critic import ClsLoss, NeigborLoss, SupConLoss, ZINBLoss
...
class PartialLabelLearning(pl.LightningModule):
    ...
    def Setup(self, encoder, pretrain=False, simu_param=None, logger=None, dataset=None):
        ...
        try:
            ...
            self.part_mat = self.dataset.getPartialLabel()  # Get candidate label matrix
            self.train_loader = self.dataset.load(self.param.batch_size)  # Get training data loader
        ...
        pretrain_model = encoder(
            num_class=self.param.num_class,
            encoder_dim=self.param.enc_dim,
            data_dim=self.param.num_features,
            latent_dim=self.param.latent_dim,
            decoder_dim=self.param.dec_dim,
            novel_cell=self.param.novel_cell,
        )  # Initialize encoder
        if pretrain:
            trainer = pl.Trainer(
                max_epochs=self.param.pretrain_epoch,
                logger=logger,
                devices=self.param.device,
                callbacks=[
                    LearningRateFinder(num_training_steps=100, max_lr=self.param.lr * 10, min_lr=self.param.lr / 10),
    ...
    def setup(self, stage):
        ...
        if stage == "fit":
            param_dic = {
                ...
                "part": self.param.partial_rate,
                "dec_dim": self.param.dec_dim,
                "enc_dim": self.param.enc_dim,
                "latent_dim": self.param.latent_dim,
                "recon_loss_weight": self.param.loss_weight,
            }
    ...
    def training_step(self, batch, batch_index):
        ...
        if len(index) != self.param.batch_size:
            return  # Skip the last batch if it is not full
        ...
        if self.hasnovel:  # If novel cell is detected
            loss_neighbor = self.critic["Neighbor"](
                latent=latent[: self.param.batch_size], proto_assign=proto_assign, cls_out=cls_out
            )
        else:
            loss_neighbor = 0
        ...
        loss = (
            (loss_cls + self.param.loss_weight * loss_cont)
            + loss_recons * self.param.recon_weight
            + 0.5 * loss_neighbor
        )
    ...
    def on_train_epoch_start(self):
        ...
        if self.current_epoch == self.param.epochs[1]:
            ...
            try:
                ...
                print("Label cluster generation failed")
                return
    ...
    def train_dataloader(self) -> TRAIN_DATALOADERS:
        ...
        if self.param.novel_cell and self.current_epoch in self.param.novel_epoch:
            ...
            if hasnovel:
                ...
                print(
                    f"Novel cell detected, updating dataloaders, current threshold:{threshold}, confident target cells:{len(masked_index)}"
                )
    ...
    def getPredictions(self, pred_outputs):
        ...
        final_adata.obs["cell_type"] = pd.Categorical(
            final_data["cell_type"], categories=np.arange(len(cell_dic))
        ).rename_categories(cell_dic)
        final_adata.obs["pred"] = pd.Categorical(
            final_data["pred"], categories=np.arange(len(cell_dic))
        ).rename_categories(cell_dic)
    ...
    def validateNovelCell(self, debug=True, pred=False):
        ...
        dataloader = self.dataset.load(batch_size=self.param.batch_size, train=False) if pred else self.train_loader
